...
/** 分页获取图文动态 POST /updates/inPage */
export async function getInPageUsingPost(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.getInPageUsingPOSTParams,
...
/** 分页获取参与的图文动态 POST /updates/inPage */
export async function getTakePartInByPageUsingPost(
    // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
    params: API.getTakePartInByPageUsingPostParams,
    body: API.UpdateSearchDto,
    options?: { [key: string]: any },
) {
  return request<API.CommonResultListUpdateVo_>('/updates/take-part-in-by-page', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
